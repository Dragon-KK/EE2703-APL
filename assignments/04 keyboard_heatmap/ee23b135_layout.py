...
try:
    ...
    warnings.warn("Did you forget to place `ee23b135_parsing_tools.py` in the current directory?", stacklevel=2)
    raise import_error
...
class Layout:
    ...
    def from_file(cls, file_path: str):
        ...
        try: # Try parsing the file and raise an error if it couldn't
            ...
            warnings.warn(f"The file `{file_path}` is not a valid json file!", stacklevel=2)
            raise json_error
        ...
        return cls.from_dict(
            keys=raw_layout["keys"],
            characters=raw_layout["characters"]
        )
