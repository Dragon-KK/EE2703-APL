...
if TYPE_CHECKING:
    from ee23b135_layout import Layout, Key, Position
...
BASIC_FINGER_MOVEMENT_CALCULATION = True
"""
Set to True to do distance calculation as per the quiz given on 30 September.
When set to False, does distance calculation as specified in the report :)
"""
...
class KeyboardAnalyzer:
    def __init__(self, layout: Layout):
        ...
        self._finger_locations: list[Key] = self._layout.group_home.copy()
        self._key_frequency = [0] * len(self._layout.position)
        self._distance_travelled = 0.0

        self._keyboard_image, self._key_centres = self._generate_keyboard()
        ...
        *NOTE*: This changes the state of the analyzer
        ...
        for char in text:
            ...
            for key in self._layout.character_map[char]:
                group = self._layout.group[key] # The finger responsible to type the letter
                start_key = self._finger_locations[group] # The current position of said finger
                ...
                if not BASIC_FINGER_MOVEMENT_CALCULATION:
                    # In the model I'm suggesting, fingers remain at the key they last typed
                    self._finger_locations[group] = key              
            
                if IGNORE_SPACE and any(v.lower().startswith("space") for v in self._layout.key_text[key]):
                    # Do not increment frequency if the character was special
                    continue

                if IGNORE_SHIFT and any(v.lower().startswith("shift") for v in self._layout.key_text[key]):
                    # Do not increment frequency if the character was special
                    continue
    ...
    def generate_heatmap(self) -> Image.Image:
        ...
        for (x, y), freq in (
            (self._key_centres[key], self._key_frequency[key])
            ...
            FUZZ = int(
                (KEY_PREFERRED_HEIGHT * HEATMAP_QUALITY) * # The radius must be proportional to the key size (in heatmap space)
                min(total_frequency / 15, 1.5) * # I wanted the heatmap to be more tame for text with less text
                min(max(0.3, freq * 1.5 / mean_frequency), 1.1) # A key will have a higher radius of influence the more frequent it was used in proportion to the others
            )
        ...
        heatmap = (heatmap // HEATMAP_CONTOURING) * HEATMAP_CONTOURING + (255 % HEATMAP_CONTOURING)        
        ...
        blurred_image = ( # Create an image (after inverting the above created heatmap)
            ...
            .filter( # Apply a gaussian blur to make it smoother
                ImageFilter.GaussianBlur(
                    ...
                    KEY_PREFERRED_HEIGHT / min(100, 4 * HEATMAP_CONTOURING)
                )
        ...
        medium_mask = np.greater(blurred_image_data[:, :, 3], LOW_CUTOFF) * (high_mask == 0)
        low_mask    = (high_mask | medium_mask == 0)
        
        # Normalize the values in each group to be between 0 to 1
        high_normalized     = (blurred_image_data[:, :, 3] - HIGH_CUTOFF) / (255 - HIGH_CUTOFF)
        medium_normalized   = (blurred_image_data[:, :, 3] - LOW_CUTOFF) / (HIGH_CUTOFF - LOW_CUTOFF)
        ...
        # orangeish:        #efef78
        # reddish:          #df4b66
        blurred_image_data[:, :, 0] = np.minimum(255, blurred_image_data[:, :, 3] + high_mask * (high_normalized * (0xdf - 0xef) + (0xef)))
        blurred_image_data[:, :, 1] = np.minimum(255, blurred_image_data[:, :, 1] + high_mask * (high_normalized * (0x4b - 0xef) + (0xef)))
        blurred_image_data[:, :, 2] = np.minimum(255, blurred_image_data[:, :, 2] + high_mask * (high_normalized * (0x66 - 0x78) + (0x78)))
        
        # Medium values go from greenish to yellowish
        # green:            #4fc46c
        # yellowish:        #efef78
        blurred_image_data[:, :, 0] = np.minimum(255, blurred_image_data[:, :, 0] + medium_mask * (medium_normalized * (0xef - 0x4f) + (0x4f)))
        blurred_image_data[:, :, 1] = np.minimum(255, blurred_image_data[:, :, 1] + medium_mask * (medium_normalized * (0xef - 0xc4) + (0xc4)))
        blurred_image_data[:, :, 2] = np.minimum(255, blurred_image_data[:, :, 2] + medium_mask * (medium_normalized * (0x78 - 0x6c) + (0x6c)))

        # # Low values go from violet to light blueish
        # violet:           #450c53
        # light blueish:    #5bb1ce
        blurred_image_data[:, :, 0] = np.minimum(255, blurred_image_data[:, :, 0] + low_mask * (low_normalized * (0x5b - 0x45) + (0x45)))
        blurred_image_data[:, :, 1] = np.minimum(255, blurred_image_data[:, :, 1] + low_mask * (low_normalized * (0xb1 - 0x0c) + (0x0c)))
        blurred_image_data[:, :, 2] = np.minimum(255, blurred_image_data[:, :, 2] + low_mask * (low_normalized * (0xce - 0x53) + (0x53)))
        ...
        return Image.alpha_composite(self._keyboard_image, Image.fromarray(blurred_image_data))
    ...
    def _generate_keyboard(self) -> tuple[Image.Image, list[Position]]:
        ...
        canvas.rounded_rectangle(
            ((0, 0), KEYBOARD_SIZE),
            radius=KEYBOARD_BBOX_PADDING, fill=COLOR_SCHEME["base"]
        )
        ...
        _map_coordinates, xscale, yscale = _deduce_space_mapping(self._layout, row_layout)

        # The centre of each key in image space (Populated later)
        key_centres: list[Position] = [(0, 0)] * len(self._layout.position)
        ...
        for row_idx in range(len(rows)):
            ...
            for col_idx in range(len(row)):
                ...
                canvas.rounded_rectangle(
                    ...
                    fill=COLOR_SCHEME["key"], radius=key_height//8
                )
...
def _get_preferred_width(key: Key, layout: Layout):
    """
...
def _deduce_space_mapping(
    ...
    right_bound = max( # Get the right bound in input coordinate space
        layout.position[row[-1]][0] + (_get_preferred_width(row[-1], layout)*approx_x_scaler)
    ...
    x_scaling_factor = (KEYBOARD_SIZE[0] - 2 * KEYBOARD_BBOX_PADDING) / (right_bound - minx)
    y_scaling_factor = (KEYBOARD_SIZE[1] - 2 * KEYBOARD_BBOX_PADDING) / (maxy - bottom_bound)
    ...
    def mapping(input_position: Position) -> Position:
        ...
        return (
            ...
            KEYBOARD_SIZE[1] - ((input_position[1] - bottom_bound) * y_scaling_factor + KEYBOARD_BBOX_PADDING), 
        )
    
    return mapping, x_scaling_factor, y_scaling_factor

def _get_rowwise_layout(
